...
-- 기존 entities JSON 파일을 기반으로 생성

-- 1. 캠페인 테이블 (campaigns.json 기반)
...
-- 3. 사용자 프로필 테이블 (user_profiles.json 기반)
CREATE TABLE IF NOT EXISTS user_profiles (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    user_id VARCHAR(255) REFERENCES users(user_id) ON DELETE CASCADE,
    signup_code VARCHAR(50),
    phone VARCHAR(20),
...
-- 4. 사용자 신청 테이블 (user_applications.json 기반)
...
-- 5. 사용자 리뷰 테이블 (user_reviews.json 기반)
...
-- 7. 포인트 히스토리 테이블 (points_history.json 기반)
...
-- 9. 알림 테이블 (notifications.json 기반)
...
-- 10. 관리자 알림 테이블 (admin_notifications.json 기반)
...
-- 11. 출금 요청 테이블 (withdrawal_requests.json 기반)
...
-- 12. 체험단 코드 테이블 (experience_codes.json 기반)
...
-- 13. 체험단 배정 테이블 (experience_assignments.json 기반)
CREATE TABLE IF NOT EXISTS experience_assignments (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    user_id VARCHAR(255) REFERENCES users(user_id) ON DELETE CASCADE,
    campaign_id UUID REFERENCES campaigns(id) ON DELETE CASCADE,
...
-- 14. 체험단 캠페인 테이블 (experience_campaigns.json 기반)
...
-- 15. 사용자 체험단 테이블 (user_experiences.json 기반)
...
-- 16. 사용자 코드 테이블 (user_codes.json 기반)
...
-- 17. 인플루언서 프로필 테이블 (influencer_profiles.json 기반)
...
-- 18. 리뷰 제출 테이블 (review_submissions.json 기반)
...
CREATE INDEX IF NOT EXISTS idx_user_applications_campaign_id ON user_applications(campaign_id);
CREATE INDEX IF NOT EXISTS idx_user_reviews_user_id ON user_reviews(user_id);
CREATE INDEX IF NOT EXISTS idx_user_reviews_campaign_id ON user_reviews(campaign_id);
...
CREATE INDEX IF NOT EXISTS idx_experience_codes_campaign_id ON experience_codes(campaign_id);
...
-- 사용자별 데이터 접근 정책
CREATE POLICY "Users can read their own data" ON user_applications FOR SELECT USING (auth.uid()::text = user_id);
CREATE POLICY "Users can read their own reviews" ON user_reviews FOR SELECT USING (auth.uid()::text = user_id);
CREATE POLICY "Users can read their own points" ON user_points FOR SELECT USING (auth.uid()::text = user_id);
CREATE POLICY "Users can read their own notifications" ON notifications FOR SELECT USING (auth.uid()::text = user_id);
CREATE POLICY "Users can read their own withdrawal requests" ON withdrawal_requests FOR SELECT USING (auth.uid()::text = user_id);
CREATE POLICY "Users can read their own experiences" ON user_experiences FOR SELECT USING (auth.uid()::text = user_id);
CREATE POLICY "Users can read their own codes" ON user_codes FOR SELECT USING (auth.uid()::text = user_id);
CREATE POLICY "Users can read their own influencer profiles" ON influencer_profiles FOR SELECT USING (auth.uid()::text = user_id);
...
CREATE POLICY "Admins can do everything" ON review_submissions FOR ALL USING (true);

-- 관리자 전용 테이블 정책 (서버에서만 접근 가능)
...
-- 완료 메시지
SELECT 'Supabase 테이블이 성공적으로 생성되었습니다!' as message;
